...
//       const response = await axios.put(`http://localhost:8000/categories/${editedCategory._id}`, formData, {
//         headers: {
//           'Content-Type': 'multipart/form-data'
//         }
//       });
...
export default function CategoryFormModal({ show, onClose, fetchCategories, editedCategory }) {
  ...
  const updateCategories = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:8000/categories/${_id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
